...
tt=0
t = 0
for str in ['jm1', 'kc3', 'cm1', 'pc3', 'pc4', 'mw1', 'pc1', 'pc5', 'mc1', 'pc2']:
    print ('-----------------------------------------------------------' + 'run' + str + '---------------------')
    ...
    def evaluat_model(model, k_folds, t=t, tt=tt):
        ...
        r1=sum_auc_M / (10 * k_folds)
        r2=sum_bal_M / (10 * k_folds)
        r3=sum_Gmean2_M / (10 * k_folds)
        tt = tt + 1
        tt_list=[r1]
...
temp_result.to_csv('temp_r.csv',index=False)
# shut down jvm
# jvm.stop()
