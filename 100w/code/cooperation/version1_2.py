...
if __name__=="__main__":
    ...
    if False:
        ...
        def make_model3():
            ...
            model=Sequential([
            Dense(44,input_dim=44, kernel_initializer=initializers.he_normal(), kernel_regularizer=regularizers.l2(0.001),
...
mb = LGBMClassifier(boosting_type='dart', is_unbalance=True, objective='xentropy',
                        seed=1239,colsample_bytree=0.69,drop_rate=0.062,learning_rate=0.03,
                    max_drop=51,n_estimators=750,num_leaves=64,reg_alpha=0.9,reg_lambda=1.5,skip_drop=0.42,
                    subsample=0.9,subsample_freq=1)
def getbest(n,train_x,train_y,turns=1):
    ips=[]
    for i in range(turns):
        m = LGBMClassifier(boosting_type='dart', is_unbalance=True, objective='xentropy',
                            seed=i+1239, colsample_bytree=0.69, drop_rate=0.062, learning_rate=0.03,
                            max_drop=51, n_estimators=750, num_leaves=64, reg_alpha=0.9, reg_lambda=1.5, skip_drop=0.42,
                            subsample=0.9, subsample_freq=1)
        ips.append (pd.Series(m.fit(train_x, train_y).feature_importances_, index=train_x.columns))
    return pd.concat(ips,1).mean(1).sort_values(ascending=False)[:n]
