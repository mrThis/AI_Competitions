...
def preprocess(x):
    ...
    # TODO:未删除
    # 相关性大的相减构建新特征
    x['FTR37-FTR46'] = x.FTR37 - x.FTR46
    x['FTR16-FTR47'] = x.FTR16 - x.FTR47
    x['FTR7-FTR4'] = x.FTR7 - x.FTR4
    x['FTR43-FTR9'] = x.FTR43 - x.FTR9
    x['FTR12-FTR38'] = x.FTR12 - x.FTR38
...
if False:
    ...
    # 观察处理后结果
    train_x.to_csv('../data/output/train_x.csv', index=False)
...
# select_feature = SelectKBest(chi2, k=100).fit(train_x, train_y)
# train_x = select_feature.transform(train_x)
# test_x = select_feature.transform(test_x)
...
rfecv = RFECV(estimator=model_lgb, step=1, cv=5,scoring='roc_auc')   #5-fold cross-validation
